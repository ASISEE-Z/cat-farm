...
func _ready() -> void:
	navigation_agent_2d.velocity_computed.connect(on_safe_velovity_computed)

	call_deferred("charater_setup")


func charater_setup() -> void:
...
func on_safe_velovity_computed(safe_velocity: Vector2) -> void:
	animated_sprite_2d.flip_h = safe_velocity.x < 0.0
